import { useState, type FormEvent } from 'react'
import { supabase } from '../lib/supabase'
import { useAuth } from '../context/AuthContext'

type Status = 'idle' | 'loading' | 'error'

export default function Onboarding() {
  const { user, refreshOrg } = useAuth()
  const [churchName, setChurchName] = useState('')
  const [status, setStatus] = useState<Status>('idle')
  const [errorMessage, setErrorMessage] = useState('')

  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    e.preventDefault()
    if (!churchName.trim() || !user) return

    setStatus('loading')
    setErrorMessage('')

    // 1. Create the org
    const { data: org, error: orgError } = await supabase
      .from('orgs')
      .insert({ name: churchName.trim() })
      .select('id')
      .single()

    if (orgError || !org) {
      setErrorMessage(orgError?.message ?? 'Failed to create church. Please try again.')
      setStatus('error')
      return
    }

    // 2. Add the current user as an admin member of that org
    const { error: memberError } = await supabase
      .from('org_members')
      .insert({ org_id: org.id, user_id: user.id, role: 'admin' })

    if (memberError) {
      setErrorMessage(memberError.message)
      setStatus('error')
      return
    }

    // 3. Refresh auth context so orgId is now populated — this will trigger
    //    the router in App.tsx to redirect away from Onboarding automatically.
    await refreshOrg()
  }

  return (
    <div className="min-h-screen bg-white flex flex-col items-center justify-center px-6">
      <div className="w-full max-w-sm">
        {/* Logo / Wordmark */}
        <div className="mb-10 text-center">
          <div className="inline-flex items-center justify-center w-14 h-14 rounded-2xl bg-indigo-600 mb-4">
            <svg
              viewBox="0 0 24 24"
              fill="none"
              stroke="currentColor"
              strokeWidth="2.5"
              strokeLinecap="round"
              className="w-7 h-7 text-white"
            >
              <line x1="12" y1="3" x2="12" y2="21" />
              <line x1="5" y1="9" x2="19" y2="9" />
            </svg>
          </div>
          <h1 className="text-3xl font-bold text-gray-900 tracking-tight">Welcome to Chapel</h1>
          <p className="mt-1 text-sm text-gray-500">Let's set up your church</p>
        </div>

        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label htmlFor="churchName" className="block text-sm font-medium text-gray-700 mb-1.5">
              Church name
            </label>
            <input
              id="churchName"
              type="text"
              autoComplete="organization"
              placeholder="e.g. Grace Community Church"
              value={churchName}
              onChange={e => setChurchName(e.target.value)}
              required
              autoFocus
              className="block w-full rounded-xl border border-gray-300 bg-white px-4 py-3 text-sm text-gray-900 placeholder-gray-400 shadow-sm focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 transition"
            />
          </div>

          {status === 'error' && (
            <p className="text-sm text-red-600 bg-red-50 border border-red-200 rounded-lg px-3 py-2">
              {errorMessage || 'Something went wrong. Please try again.'}
            </p>
          )}

          <button
            type="submit"
            disabled={status === 'loading' || !churchName.trim()}
            className="w-full rounded-xl bg-indigo-600 px-4 py-3 text-sm font-semibold text-white shadow-sm hover:bg-indigo-700 active:bg-indigo-800 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
          >
            {status === 'loading' ? 'Creating…' : 'Create my church'}
          </button>

          <p className="text-center text-xs text-gray-400 pt-2 leading-relaxed">
            You'll be the admin. You can add team members after setup.
          </p>
        </form>
      </div>
    </div>
  )
}
