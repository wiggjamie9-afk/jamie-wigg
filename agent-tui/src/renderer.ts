...
type Pending = { name: string; callId: string; args: Record<string, unknown>; output?: string };
...
export class TuiRenderer {
  ...
  private renderToolResult(_name: string, callId: string, output: string): void {
    if (this.display.toolDisplay === 'hidden') return;

    if (this.display.toolDisplay === 'emoji') {
      const ms = Date.now() - (this.toolStart.get(callId) ?? Date.now());
      const pending = _name;
      console.log(`  ${GREEN}✓${RESET} ${DIM}${pending} (${(ms / 1000).toFixed(1)}s)${RESET}`);
    } else if (this.display.toolDisplay === 'grouped') {
  ...
  private flushMinimal(): void {
    ...
    for (const [name, count] of this.minimalBatch) {
      ...
      if (label) parts.push(`${label.past.toLowerCase()} ${plural(count, label.noun)}`);
      else parts.push(`${plural(count, name)}`);
    }
