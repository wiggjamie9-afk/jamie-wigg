---
name: notebooklm-py
description: Canonical teng-lin/notebooklm-py — complete unofficial API/CLI/MCP for Google NotebookLM. Create notebooks, bulk-add sources (URL/PDF/YouTube/Drive), grounded cited Q&A (RAG), web/Deep research auto-import, and generate ALL artifacts (podcast audio, video, slide deck, quiz, flashcards, infographic, report, data table, mind map) with multi-format download. Also enables a free cross-session "Master Brain" memory and zero-token research offload. Unofficial (undocumented Google APIs, may break); needs Google login + Chromium; runs on a real machine/Codespace, not the web sandbox. Install: uv tool install "notebooklm-py[browser]"
---
