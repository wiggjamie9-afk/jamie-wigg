> ARCHIVED FULL REFERENCE — the lean day-to-day brain is in the repo root `CLAUDE.md`. This file holds the complete original technical reference (tools, MCP servers, pipelines) for when deep repo detail is needed.
...
## ⭐ JAMIE'S RULES — READ THIS FIRST, EVERY SESSION

This is Jamie's standing agreement with Claude. It is loaded automatically at the
start of every session and persists in git, so Jamie should **never have to repeat
these**. If any instruction below conflicts with a habit or a default, this wins.

**Jamie's focus:** creating things — images, videos, and working apps/sites — with
the *least possible setup and friction*. Keep Jamie pointed at the actual output he
wants, not the plumbing.

**Hard rules (do not break):**

1. **Never spend money or credits without asking first.** Image generation, video
   generation, paid APIs, anything billable — STOP and get an explicit "yes" before
   running it. Never fire a paid action to "prove it works."
2. **No setup runarounds.** Do not send Jamie off to install, plug in, pull, or
   configure things. If something genuinely can't be done right here, say so in one
   plain sentence and stop — don't hand him a checklist.
3. **Do exactly what was asked.** No side-quests, no "while I'm here" extras, no
   wandering. If the request is unclear, make the smallest reasonable assumption and
   act — don't bury him in questions.
4. **Be honest about limits up front.** Claude Code is a coding tool; it has no free
   built-in image/video generation. Say what's actually possible *before* doing
   anything, so Jamie isn't led in circles.
5. **Keep Jamie focused.** If he's drifting or looping, name the one main goal and
   give the single next step. Short. Concrete.
6. **Remember, don't re-ask.** These rules and anything Jamie tells me to "remember"
   live here. Read them; act on them; don't make him say them twice.

**Tone:** direct, no fluff, no over-apologising, no walls of text. Give the answer
and the next step.

*To change any rule: Jamie just says so, and Claude edits this block.*
...
- **Zyloo MCP server (OpenAI-compatible LLM gateway)** → `.claude/mcp/zyloo/server.mjs`, registered in `.mcp.json` as `zyloo`. Zyloo fronts Claude and other frontier models through the OpenAI Chat Completions wire format, so any routed model is reachable via `/chat/completions` and `/models`. Put `ZYLOO_API_KEY` (key starts with `sk-zy-`) in `.env` — `ZYLOO_BASE_URL` (default `https://api.zyloo.io/v1`) and `ZYLOO_MODEL` (default `zyloo/claude-haiku-4-5-20251001`) are optional. Run `cd .claude/mcp/zyloo && npm install` once. Exposes three tools: `zyloo_complete` (single-prompt completion), `zyloo_chat` (multi-turn + optional `user_images` for vision), `zyloo_list_models` (discover routable model ids). See `.claude/mcp/zyloo/README.md`.
- **Gemini MCP server (text + native image generation via ZenMux)** → `.claude/mcp/gemini/server.mjs`, registered in `.mcp.json` as `gemini`. Routes Google Gemini models through the ZenMux Vertex-AI gateway (Google Gen AI `generateContent` wire format, Bearer auth). Put `ZENMUX_API_KEY` (key starts with `sk-`) in `.env` — `ZENMUX_BASE_URL` (default `https://zenmux.ai/api/vertex-ai`) and `ZENMUX_GEMINI_MODEL` (default `google/gemini-3.1-flash-lite-image`) are optional. Run `cd .claude/mcp/gemini && npm install` once. Exposes six tools. **Gemini-native** (`generateContent` / Vertex-AI path): `gemini_generate` (single-prompt text), `gemini_chat` (multi-turn `contents` format + optional `user_images` URLs for vision), `gemini_image` (native image generation → saves PNG(s) to `creative-out/`, override via `GEMINI_OUT_DIR`). **OpenAI-compatible** (`{base}/api/v1/chat/completions`, routes any model — Claude/GPT/Kimi/Doubao/Gemini): `zenmux_complete` (single prompt), `zenmux_chat` (multi-turn OpenAI `messages` + optional `user_images`). Plus `gemini_list_models` (discover routable model slugs). Model slugs are provider-namespaced, e.g. `google/gemini-3.1-flash-lite-image`, `anthropic/claude-opus-4.8`. `zenmux_*` default model is `anthropic/claude-sonnet-5-free` (override via `ZENMUX_CHAT_MODEL`). ⚠️ The sandbox egress allowlist blocks `zenmux.ai` — tools register fine; only runtime is gated. See `.claude/mcp/gemini/README.md`.
...
| `zyloo` | `node .claude/mcp/zyloo/server.mjs` | Zyloo OpenAI-compatible gateway fronting Claude + other models. Tools: `zyloo_complete`, `zyloo_chat`, `zyloo_list_models`. Needs `.env`: `ZYLOO_API_KEY` (`ZYLOO_BASE_URL`, `ZYLOO_MODEL` optional). |
| `gemini` | `node .claude/mcp/gemini/server.mjs` | ZenMux gateway: Gemini-native text + image gen (`gemini_generate`, `gemini_chat`, `gemini_image`) **and** OpenAI-compatible chat to any routed model — Claude/GPT/Kimi/Doubao/Grok (`zenmux_complete`, `zenmux_chat`), plus `gemini_list_models`. Needs `.env`: `ZENMUX_API_KEY` (`ZENMUX_BASE_URL`, `ZENMUX_GEMINI_MODEL`, `ZENMUX_CHAT_MODEL` optional). |
