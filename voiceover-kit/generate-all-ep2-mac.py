#!/usr/bin/env python3
"""
Generate all 20 Episode 2 MP3s using local voice samples + Voicebox.

This script:
1. Reads voice samples from voice-samples/ folder (same directory as this script)
...
- All voice samples in voice-samples/ folder (extract voice-samples-20-podcasts.tar.gz)

Usage:
    python3 generate-all-ep2-mac.py
"""
...
VOICE_SAMPLES_DIR = SCRIPT_DIR / "voice-samples"
EP2_DIR = SCRIPT_DIR / "audio-ep2"

# Voice mapping: podcast_key → (voice_name, file_id_prefix)
VOICES = {
...
def find_voice_file(file_id_prefix):
    """Find the voice sample MP3 by file ID prefix in voice-samples/ folder."""
    if not VOICE_SAMPLES_DIR.exists():
        return None

    for file in VOICE_SAMPLES_DIR.glob(f"{file_id_prefix}*.mp3"):
        return file
    return None


def clone_voice(podcast_key, voice_name, file_id_prefix):
    """Clone a voice from the local voice sample."""
    voice_file = find_voice_file(file_id_prefix)
    if not voice_file:
        print(f"  ⚠️  {podcast_key:35} (voice sample not found in voice-samples/)")
...
def read_script(podcast_key):
    ...
    script_path = SCRIPT_DIR / f"{podcast_key}-ep2.txt"
...
def main():
    print("\n" + "=" * 70)
    print("🎙️  EPISODE 2 GENERATION: Local Voice Samples → All 20 Podcasts")
    print("=" * 70 + "\n")

    # Check voice samples folder
    if not VOICE_SAMPLES_DIR.exists():
        print("❌ voice-samples/ folder not found!")
        print("   Extract voice-samples-20-podcasts.tar.gz first:")
        print("   tar -xzf voice-samples-20-podcasts.tar.gz")
        print("   This creates voice-samples/ folder with all 20 MP3s\n")
        return
    ...
    # Phase 1: Clone voices
    print("PHASE 1: Cloning voices from local samples")
    print("-" * 70)
    ...
    # Phase 2: Generate Episode 2
    print("PHASE 2: Generating all 20 Episode 2 MP3s")
    print("-" * 70)
