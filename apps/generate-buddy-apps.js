...
<body>
    <div class="app">
        ...
        <div class="tabs">
            ...
            <button class="tab" onclick="app.switchTab('memory')">🧠 Memory</button>
        ...
        <div class="content">
            ...
            <div class="screen" id="memoryScreen">
                <div style="font-size: 14px; color: var(--text-secondary); margin-bottom: 12px;">🧠 What ${buddy.name} remembers about you</div>
                <div style="font-size: 12px; color: var(--text-tertiary); margin-bottom: 12px;">Stored only on your device. You control it.</div>
                <ul id="memoryList" style="list-style: none; margin: 0; padding: 0; gap: 8px; display: flex; flex-direction: column;"></ul>
                <button class="button" onclick="app.clearMemory()" style="margin-top: 16px; background: rgba(251,113,133,0.1); color: #fda4af; border-color: rgba(251,113,133,0.3);">Clear All Memory</button>
            </div>
    ...
    <script>
        ...
        const app = {
            ...
            loadMemory() {
                try {
                    const stored = localStorage.getItem('buddy_memory_' + BUDDY.id);
                    if (stored) this.memory = JSON.parse(stored);
                } catch (e) { /* keep default memory */ }
            },

            saveMemory() {
                localStorage.setItem('buddy_memory_' + BUDDY.id, JSON.stringify(this.memory));
            },
            ...
            async updateMemory() {
                ...
                const memSys = 'You maintain a compact long-term memory about a user. Extract only durable, useful facts (name, preferences, ongoing situations, goals). Return ONLY JSON: {\"summary\": string, \"facts\": [string]}. Keep facts to 12 max.';
                const memUser = 'Current memory: ' + JSON.stringify(this.memory) + '\\n\\nRecent conversation:\\n' + convo + '\\n\\nReturn updated memory JSON only.';
                try {
                    const response = await this.callClaude(memSys, [{role: 'user', content: memUser}]);
                    const match = response.match(/\\{[\\s\\S]*\\}/);
                    if (match) {
                        const parsed = JSON.parse(match[0]);
                        if (parsed && Array.isArray(parsed.facts)) {
                            this.memory = { summary: parsed.summary || '', facts: parsed.facts.slice(0, 12) };
                            this.saveMemory();
                        }
                    }
                } catch (e) { /* keep existing memory */ }
            },
            ...
            switchTab(tab) {
                ...
                if (tab === 'memory') this.renderMemory();
            },
            ...
            async sendMessage() {
                ...
                try {
                    ...
                    this.turnsSinceMemUpdate++;
                    if (this.turnsSinceMemUpdate >= 3) {
                        this.turnsSinceMemUpdate = 0;
                        this.updateMemory();
                    }
                } catch (err) {
                    typingMsg.remove();
            ...
            async callClaude(userMessage, customSystem = null) {
                let systemPrompt = customSystem || \`You are \${BUDDY.name}, a \${BUDDY.role}. Personality: \${BUDDY.personality}. Keep responses warm, brief (1-3 sentences), supportive.\`;

                if (this.memory.facts && this.memory.facts.length > 0 && !customSystem) {
                    systemPrompt += '\\n\\nLong-term memory about this person (use naturally, do not recite):';
                    this.memory.facts.forEach(fact => { systemPrompt += '\\n- ' + fact; });
                    if (this.memory.summary) systemPrompt += '\\nSummary: ' + this.memory.summary;
                }
            ...
            renderMemory() {
                ...
                if (!this.memory.facts || this.memory.facts.length === 0) {
                    ...
                    li.textContent = 'Nothing remembered yet. As you chat, I\'ll remember durable facts.';
                    ul.appendChild(li);
                    return;
                }
                ...
                if (this.memory.summary) {
                    ...
                    summary.style.background = 'rgba(124,58,237,0.1)';
                    summary.style.border = '1px solid rgba(124,58,237,0.2)';
                    summary.style.fontSize = '13px';
                    summary.style.fontStyle = 'italic';
                    summary.textContent = '📌 ' + this.memory.summary;
