...
Every other focus/ADHD app is a plain timer bolted to a stock playlist. Undertow's differentiator
...
Local: `python3 -m http.server 8000 --directory apps/undertow` → open `http://localhost:8000`.
(A service worker needs http/https, not `file://`.)

Deploy: it's static — serve `apps/undertow/` from GitHub Pages or Cloudflare Pages. Add-to-home-screen
