#!/usr/bin/env python3
"""
Fire Higgsfield art generation jobs for Books 5-8 (80 images total).
Requires HIGGSFIELD_API_KEY and HIGGSFIELD_SECRET in .env or environment.

Usage:
  python fire-books-5-8-jobs.py --dry-run    # test without firing
  python fire-books-5-8-jobs.py              # fire all 80 jobs
"""
...
class HighgsfieldJobFirer:
    ...
    def fire_book_jobs(self, book_num: int, dry_run: bool = False) -> dict:
        ...
        print(f"\n📖 Book {book_num}: Sunny and the {'Little Bilby' if book_num == 5 else 'Kookaburra' if book_num == 6 else 'Platypus' if book_num == 7 else 'Sleepy Echidna'}")
...
def main():
    ...
    print("🎨 Firing Higgsfield jobs for Books 5-8")
    print(f"   Model: {MODEL}")
    ...
    if not dry_run:
        ...
        print("   2. Once images are ready, create art-fetch-manifest.txt with download URLs")
        print("   3. Push art-fetch-manifest.txt to trigger GitHub CI to fetch and commit images")
        print("   4. Run: python build_book_v2.py 5 6 7 8  (to assemble PDFs/EPUBs)")
