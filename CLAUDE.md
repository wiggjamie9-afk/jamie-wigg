...
- `SETUP-KORTIX-REGISTRY.md` — Kortix Registry (MIT) of AI-agent capabilities — skills/tools/plugins/agents — installed via the `ocx` CLI from `https://registry.kortix.ai` into an OpenCode project (`.opencode/`). A fourth capability channel alongside `.agents/skills/`, `.mcp.json`, and slash commands; additive, not a replacement. Registry already wired into `.opencode/ocx.jsonc`. ⚠️ `registry.kortix.ai` is egress-gated in the sandbox (`ocx` installs from npm fine; `ocx add kortix/...` only resolves on open egress). Prefer it for components not already vendored (`kortix-memory`, `kortix-tunnel`, `legal-writer`, `paper-creator`)
