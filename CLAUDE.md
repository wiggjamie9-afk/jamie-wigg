...
| `mac-downloads/` | **Mac download bundle** — `README.md` (rolling summary of recent repo additions) + `Install-Downloads.command` (unattended one-click Mac installer). Permanent home for "pull the latest tools onto the Mac". |
...
- `SETUP-OPENCODE.md` — OpenCode terminal AI coding-agent CLI reference (agent-CLI family alongside Freebuff/Hermes/Agent TARS/OpenManus); built-in `build`/`plan` agents + desktop app, alternative coding agent, not a pipeline tool
- `SETUP-SIMPLEX.md` — SimpleX Chat reference (AGPLv3); privacy-first messenger with **no user identifiers** (pairwise per-queue IDs), double-ratchet + extra NaCl layer + post-quantum key exchange, Trail of Bits-audited. Mobile/desktop apps + a terminal CLI (`curl … install.sh | bash`, wired into `mac-downloads/Install-Downloads.command`) that runs as a local WebSocket server for private bots/automations (notifiers, integrations). Reference/comms tool, not part of the video pipeline
- `SETUP-IMPECCABLE.md` — Impeccable design-quality toolkit for AI coding agents (Apache-2.0); 1 skill + 23 `/impeccable` commands + live browser mode + **44 deterministic detector rules** for AI-frontend slop. Directly relevant to `studio/`, the root site, and `sites/<slug>/` pipeline output; a parallel skill to the repo's `frontend-design`. Install via `npx impeccable install` (wired into `mac-downloads/Install-Downloads.command`); standalone CI gate via `npx impeccable detect`. Lock its `DESIGN.md` to `rhythmix-teaser-60s/DESIGN.md`
- `SETUP-VERCEL.md` — Vercel CLI (`vercel`/`vc`) reference (Apache-2.0); deploy/`vercel dev`. Tangential — this repo ships via GitHub Pages + Cloudflare Pages, not Vercel; keep for one-off/experimental deploys. Installer adds the standard `npm i -g vercel`; native binary (`@vercel/vc-native`) is opt-in
- `SETUP-AWESOME-LLM-APPS.md` — `Shubhamsaboo/awesome-llm-apps` cookbook reference (Apache-2.0); ~15 categories of self-contained, provider-agnostic LLM app templates (agents, always-on, multi-agent, voice, MCP, RAG, agent skills, memory, fine-tuning). A **parts bin to fork from** — lift one template into `automation/<slug>/` or `apps/<name>/` with attribution; don't vendor the whole repo. Installer clones it to `~/awesome-llm-apps` (source only; per-template `pip install`)
- `SETUP-GRAPHIFY.md` — Graphify knowledge-graph tool (PyPI package `graphifyy`, command `graphify`); `/graphify .` maps code/docs/PDFs/media into `graphify-out/{graph.html,GRAPH_REPORT.md,graph.json}` you query instead of grepping. **This is the tool that produces the repo's existing `graphify-out/` snapshot.** Code extracted locally via tree-sitter (no API); `graphify query/path/explain`, `graphify hook install` for commit-time rebuilds. Installer runs `uv tool install graphifyy` + `graphify install`
- `SETUP-CLAWFLEET.md` — ClawFleet reference (`github.com/clawfleet/ClawFleet`); Go CLI + browser dashboard to deploy/manage a fleet of isolated **OpenClaw** instances (XFCE4+noVNC desktop + Gateway) as Docker containers, built on the ClawSandbox infra layer. Pins a tested OpenClaw version per release. Adjacent to this repo's OpenClaw CLI skills but a separate, Docker-based self-hostable product. Wired into the Mac installer as a heavy/optional step (installs Go, clones the repo, `make build` → `~/clawfleet/bin/clawfleet`; needs Docker, ~1.4 GB image on first `dashboard`, `SKIP_HEAVY=1` skips it)
- `SETUP-ZENII.md` — Zenii reference (`github.com/sprklai/zenii`, BSD-ish); local AI backend for agents (~20 MB Rust daemon at `localhost:18981`, 133 routes, 19 tools) with **persistent semantic memory** (FTS5 + vectors), multi-provider chat, scheduler, workflows, and an **MCP server** (`zenii-mcp-server`) you can register in `.mcp.json`. Relevant as shared cross-session memory + autonomous scheduler for the repo's agents. Mac installer builds it from source under `SKIP_HEAVY` (Rust); desktop app is the lighter path. `.mcp.json` is **not** auto-edited
- `SETUP-SCRAPLING.md` — Scrapling reference (`github.com/D4Vinci/Scrapling`, BSD-3-Clause); adaptive Python web-scraping framework — self-healing parser (elements relocate on redesign via `adaptive=True`), anti-bot fetchers (Cloudflare Turnstile bypass), Scrapy-like spiders with pause/resume + proxy rotation, and a token-saving **MCP server** for AI scraping. Useful for the repo's research/automation side (respect robots.txt/ToS). Mac installer runs `pip install "scrapling[all]"` (light) + `scrapling install` for browsers (heavy, `SKIP_HEAVY`)
- `SETUP-FLOW.md` — Flow reference (`myflow.sh`, `github.com/nikivdev/flow`); developer-workflow CLI (`f`/`flow`/`lin`) wrapping the local `setup`/`test`/`deploy`/`tasks` loop, meant to be used with AI. Tangential utility (not a pipeline tool). Mac installer runs `curl -fsSL https://myflow.sh/install.sh | sh` (installs to `~/.flow/bin/f`; checksum-verified unless `FLOW_INSTALL_INSECURE=1`)
- `SETUP-GODOT.md` — Godot Engine reference (`github.com/godotengine/godot`, MIT); free/open-source cross-platform 2D+3D game engine with one-click export (desktop/mobile/web/console). Tangential to the video pipeline — a general creative tool (possible fit for `apps/untapped/` game concepts). Mac installer runs `brew install --cask godot` (light GUI app)
- `SETUP-AWESOME-CLAUDE-CODE.md` — `hesreallyhim/awesome-claude-code` reference; curated list of Claude Code skills/subagents/hooks/status-lines/orchestrators/plugins. A **discovery source** for this repo's own agent ecosystem (`.claude/skills/`, `.agents/skills/`) — browse it, then vendor picks the usual way (`.agents/skills/` + `skills-lock.json`), like the already-vendored mattpocock/anthropic skills. Link list, **not installed**. Sibling lists: ComposioHQ/travisvn `awesome-claude-skills`, `rohitg00/awesome-claude-code-toolkit`
- `SETUP-APPRISE.md` — Apprise reference (`github.com/caronc/apprise`, MIT); one API/CLI/pip library to send notifications to ~100 services (Telegram/Discord/Slack/ntfy/Gotify/email/SMS). Serves the recurring "notify when render/deploy/bundle done" need; pipeline-capable (`pip install apprise`), so the Mac installer installs it (light). Alternatives: ntfy, Gotify, Novu
- `SETUP-LISTMONK.md` — Listmonk reference (`github.com/knadh/listmonk`, AGPL-3.0, Go/Docker); self-hosted newsletter/mailing-list manager (Mailchimp alternative). Closes the loop on the Viral Hook Generator's email-list lead magnet (point its opt-in at a Listmonk list). Self-hosted Docker service — documented, not auto-installed. Alternatives: Keila, Mautic
- `SETUP-MATCHERING.md` — Matchering reference (`github.com/sergree/matchering`, GPL-3.0); automated music mastering (LANDR/eMastered alternative) — matches a target track's RMS/EQ/peak/stereo width to a reference. Ships as a **Python library** (`pip install matchering`) so it's pipeline-capable (master tracks / normalize `narration.wav` next to MoviePy); web UI is Docker. Documented, not auto-installed
- `SETUP-HANDBRAKE-WEB.md` — HandBrake Web reference (AGPL-3.0, Docker); web UI over the HandBrake transcoder for headless, queued, multi-worker video transcoding. Tail-end of the video pipeline (compress/repurpose HyperFrames MP4s to H.265/AV1); complements `SETUP-MOVIEPY.md`. Self-hosted Docker service — documented, not in the Mac installer
- `SETUP-ACTIVEPIECES.md` — Activepieces reference (`github.com/activepieces/activepieces`, MIT, Docker); no-code Zapier/n8n alternative. Parallel automation surface to the repo's n8n workflows; doc also catalogs adjacent self-hosted engines (Kestra, Huginn, StackStorm, Automatisch, changedetection.io, Healthchecks). Self-hosted Docker service — documented, not auto-installed
- `SETUP-GODMODE.md` — GodMode reference (`github.com/smol-ai/GodMode`); a small "AI chat browser" (Electron) that fans one prompt across ChatGPT/Claude/Perplexity/Bing/etc. web apps at once via **Cmd+Shift+G** (web apps, not APIs, so launch-day features work). Tangential productivity tool (compare model outputs for RHYTHMIX copy). Mac installer downloads the latest universal `.dmg` from GitHub releases → `/Applications` (unsigned — right-click→Open first run)
...
- `SETUP-PENPOT.md` — Penpot open-source, self-hostable design platform (MPL-2.0); native Design Tokens + Components/Variants, MCP server, open API/plugins, SVG/CSS/HTML export. Design-authoring layer that maps to `rhythmix-teaser-60s/DESIGN.md` and feeds `studio/` + the site-build pipeline
- `SETUP-MATT-POCOCK-SKILLS.md` — `mattpocock/skills` engineering-skills collection (install via `npx skills@latest add mattpocock/skills`); likely upstream source of the repo's `/grill-me`, `/grill-with-docs`, `/tdd`, `/diagnose`, `/to-prd`, `/to-issues`, `/triage`, `/prototype`, `/handoff`, `/improve-codebase-architecture`. Diff before re-syncing; update `skills-lock.json`
- `SETUP-ANTHROPIC-SKILLS.md` — `anthropics/skills` official Agent Skills repo (Apache-2.0 examples + source-available `docx`/`pdf`/`pptx`/`xlsx` document skills); canonical upstream for vendored skills already here (`docx`, `pdf`, `pptx`, `xlsx`, `skill-creator`, `mcp-builder`, `canvas-design`, `brand-guidelines`, `algorithmic-art`, `webapp-testing`). Includes the Agent Skills spec + `SKILL.md` template. Install via `/plugin marketplace add anthropics/skills`; on this repo prefer the `.agents/skills/` sync over the marketplace to avoid duplication
