...
- `SETUP-SIMPLEX.md` — SimpleX Chat reference (AGPLv3); privacy-first messenger with **no user identifiers** (pairwise per-queue IDs), double-ratchet + extra NaCl layer + post-quantum key exchange, Trail of Bits-audited. Mobile/desktop apps + a terminal CLI (`curl … install.sh | bash`, wired into `Install-Downloads.command`) that runs as a local WebSocket server for private bots/automations (notifiers, integrations). Reference/comms tool, not part of the video pipeline
- `SETUP-IMPECCABLE.md` — Impeccable design-quality toolkit for AI coding agents (Apache-2.0); 1 skill + 23 `/impeccable` commands + live browser mode + **44 deterministic detector rules** for AI-frontend slop. Directly relevant to `studio/`, the root site, and `sites/<slug>/` pipeline output; a parallel skill to the repo's `frontend-design`. Install via `npx impeccable install` (wired into `Install-Downloads.command`); standalone CI gate via `npx impeccable detect`. Lock its `DESIGN.md` to `rhythmix-teaser-60s/DESIGN.md`
