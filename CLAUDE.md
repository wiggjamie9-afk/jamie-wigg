...
- **Claude Code proxy (`free-claude-code` — route Anthropic API to NVIDIA NIM / Kimi / OpenRouter / Z.ai / local LM Studio / llama.cpp / Ollama)** → installed system-wide via `uv tool install` as `fcc-server`, `fcc-claude`, `fcc-init`. Source is pinned as a submodule at `vendor/free-claude-code` (hydrate with `git submodule update --init --recursive`). Setup walkthrough including a Python 3.14rc2 pydantic patch and a `tiktoken` egress note → `docs/claude-code-proxy.md`. Config lives at `~/.config/free-claude-code/.env` (not the repo-root `.env`, which is Higgsfield's). Useful when the user wants per-tier routing (Opus/Sonnet/Haiku → different upstreams) or local-model Claude Code workflows.
