...
- **Gemini MCP server (text + native image generation via ZenMux)** → `.claude/mcp/gemini/server.mjs`, registered in `.mcp.json` as `gemini`. Routes Google Gemini models through the ZenMux Vertex-AI gateway (Google Gen AI `generateContent` wire format, Bearer auth). Put `ZENMUX_API_KEY` (key starts with `sk-`) in `.env` — `ZENMUX_BASE_URL` (default `https://zenmux.ai/api/vertex-ai`) and `ZENMUX_GEMINI_MODEL` (default `google/gemini-3.1-flash-lite-image`) are optional. Run `cd .claude/mcp/gemini && npm install` once. Exposes four tools: `gemini_generate` (single-prompt text), `gemini_chat` (multi-turn `contents` format + optional `user_images` URLs for vision), `gemini_image` (native image generation → saves PNG(s) to `creative-out/`, override via `GEMINI_OUT_DIR`), `gemini_list_models` (discover routable model slugs). Model slugs are provider-namespaced, e.g. `google/gemini-3.1-flash-lite-image`. ⚠️ The sandbox egress allowlist blocks `zenmux.ai` — tools register fine; only runtime is gated. See `.claude/mcp/gemini/README.md`.
...
| `gemini` | `node .claude/mcp/gemini/server.mjs` | Google Gemini (text + native image gen) via the ZenMux Vertex-AI gateway. Tools: `gemini_generate`, `gemini_chat`, `gemini_image`, `gemini_list_models`. Needs `.env`: `ZENMUX_API_KEY` (`ZENMUX_BASE_URL`, `ZENMUX_GEMINI_MODEL` optional). |
