...
- **Context7 docs MCP** → already wired in this environment as an MCP server exposing `query-docs` and `resolve-library-id`. Use it whenever you need current library/framework/SDK documentation (React, Next.js, Tailwind, Playwright, etc.) — prefer over web search and over your own training knowledge, since training may not reflect recent versions. Not for refactoring, debugging business logic, or general programming questions.
