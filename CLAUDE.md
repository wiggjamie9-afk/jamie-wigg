...
- **Repurpose a finished Promo to portrait/square** → invoke the `promo-repurpose` skill. **Verify a Cut before publishing** → `render-verify`. **Smoke-test the live site after a deploy** → `deploy-check`. Cadence map for all of it (on-demand vs scheduled routines vs event-driven): `docs/AUTOMATION.md`.
- **Marketing data Q&A (ad spend, GA4, campaign performance)** → Supermetrics hosted MCP connector — see `SETUP-SUPERMETRICS.md`. Connected at the claude.ai level, not in `.mcp.json`.
