...
- **OpenClaw CLI skills (e.g. `ai-video-editor-motion-graphics`, `self-improving-agent`)** → installed via `openclaw skills install <slug>` from ClawHub into OpenClaw's own managed skills directory (typically `~/.openclaw/...` or per-agent workspace), **not** into this repo's `.claude/skills/`. So these don't get committed here. To install: `npx -y openclaw skills install <slug>` on a machine with unrestricted egress (ClawHub is blocked from the cloud sandbox by the egress allowlist). Discover slugs with `npx -y openclaw skills search <query>` and inspect with `npx -y openclaw skills info <slug>`. Add `--global` to install for all agent workspaces; default installs into the cwd-inferred agent only.
