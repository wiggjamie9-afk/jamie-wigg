...
- **Native iOS/Android app — two parallel scaffolds** (pick one before publishing):
  ...
  - `capacitor/` — **Capacitor v8** (no Ionic framework). Bundles `index.html`/`rhythmix.html` etc. into the binary (offline). Bundle ID `com.rhythmix.web`. Rebuild web bundle with `npm run build:www`; sync to native with `npm run sync`. **Ionic Appflow** is wired up (`ionic.config.json`, `appflow.config.json`, `@capacitor/live-updates` plugin) so iOS/Android builds can run in Appflow's cloud from a connected GitHub branch — see `capacitor/README.md` for the App ID fill-in and signing-cert upload steps. Best for fastest store submission of the existing site.
...
- **Self-host a personal ChatGPT (Open WebUI)** → `tools/open-webui/` has a `docker-compose.yml` + setup notes. Parked for when you have a Mac mini or a $5/mo VPS — not runnable from iPhone alone.
