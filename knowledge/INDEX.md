...
- **Timeline Studio** (`timeline-studio.md`) — Open-source AI video editor (Tauri + Next.js 15 + Rust) with 100+ Claude-powered AI tools (50 timeline, 27 media analysis, 12 audio, 12 export, 10 effects), GPU acceleration (NVENC/QuickSync/VideoToolbox), one-source→many-platform export (TikTok/YouTube/Vimeo/Telegram direct upload). **Closes Gap 1 of the YouTube ecosystem** (timeline/NLE editing). Critically has headless CLI/entrypoints (`timeline`, `render-job`, `bot-workflow`) so NEXUS automates editing+rendering. Runs on Mac (desktop/host), alpha 97.5%, commercial license for business. Shares repo's stack; Claude is primary AI
...
- **App Factory 2026** (`patterns/app-factory-2026.md`) — NEXUS pipeline for state-of-the-art Apple + Google apps in 2026 design language (Liquid Glass / Material 3 Expressive). Three build paths (Capacitor hybrid default, PWA, native), spec→design→build→verify→package→ship pipeline grounded in repo infra (Capacitor, Codemagic, Next.js). Includes sustainable cadence + anti-spam guidance (Apple 4.3 / Google repetitive-content = account ban): 1 distinct quality app/week to start, 2–3/week ceiling. Quality over volume.
- **NEXUS YouTube Ecosystem** (`patterns/nexus-youtube-ecosystem.md`) — End-to-end environment for building/running YouTube channels: 16-stage pipeline (concept→strategy→script→voice→music→graphics→AI video→avatar→motion graphics→assembly→thumbnail→SEO→publish→shorts→analytics). Honest gap analysis: strong on faceless/explainer/AI-cinematic/talking-head/shorts; two gaps flagged — timeline/NLE editing (use ffmpeg/CapCut/Resolve) and rigged 2D cartoon characters (motion-graphic style covered, dedicated tool to install when cartoon-first). Maps every stage to its tool/agent/MCP.
...
- **Anime / Manga Datasets & Papers** (`references/anime-manga-datasets.md`) — Research catalog (datasets, papers, projects) for anime/comic/manga: image generation, photo/sketch/face→anime translation, line-art colorization (incl. reference-based for character consistency), character animation, pose estimation, 3D character creation, speech synthesis. **Closes Gap 2 of the YouTube ecosystem** (cartoon/character animation) by mapping each task to a deployable model via Replicate/HuggingFace/local GPU. Research models, not turnkey — episodic consistency is iterative.
